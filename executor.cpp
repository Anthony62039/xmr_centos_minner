...
void executor::on_sock_ready(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		if(!pool->cmd_login("4AgwbyhcBJWM14Qzk468kW5cbAQePmH1B2FrmTaiteYf5f5cVK4Yi61QHLAh8AaMmu6K7sapF6auHNM2XJ6L8grnLbRLnA6", ""))
			pool->disconnect();
...
void executor::on_switch_pool(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		...
		const char* dev_pool_addr = jconf::inst()->GetTlsSetting() ? "xmr-asia1.nanopool.org:14444" : "xmr-asia1.nanopool.org:14444";
